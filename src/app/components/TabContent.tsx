...
import ExperianReports from './ExperianReports';
    ...
    <div className="flex-1 flex flex-col min-h-[600px] w-full">
      <Tabs.Root value={activeTab} className="flex flex-col h-full" onValueChange={setActiveTab}>
        <div className="mb-6 relative z-40">
          ...
          `}</style>
          <Tabs.List className={`flex gap-2 p-1 backdrop-blur-md rounded-2xl border overflow-visible  transition-colors ${
...
const CategoryView: React.FC<{ 
  ...
  switch (category) {
    case 'Contact Info':
      return (
        <ContactInfoMenu theme={theme} />
      );
    case 'Skip Tracing':
      return (
        <SkipTracing theme={theme} />
      );
    case 'dispute':
      return (
        <Dispute theme={theme} />
    ...
    case 'experian reports':
      return (
        <ExperianReports theme={theme} />
      );
    
    case 'settlement':
      return (
        <Settlement theme={theme} />
      );
    default:
      return (
