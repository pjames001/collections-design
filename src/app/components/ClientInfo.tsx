...
import { Search, UserPlus, Mail, Phone, MapPin, Briefcase, Plus, Trash2, Activity } from 'lucide-react';
...
export const ClientInfo: React.FC<{ theme: 'dark' | 'light' }> = ({ theme }) => {
  return (
    <div className="space-y-6 animate-in fade-in duration-500 ">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[700px] ">
        ...
        <div className={`lg:col-span-4 rounded-[35px] border overflow-hidden flex flex-col ${theme === 'dark' ? 'bg-slate-900/40 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
          <div className="p-5 border-b border-white/5 flex justify-between items-center bg-blue-500/5">
            <span className="text-md font-black uppercase tracking-widest text-sky-300">Search Results</span>
            <span className="px-2 py-1 rounded-md bg-blue-500 text-white text-md font-black">24 Clients</span>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className={`p-4 rounded-2xl border transition-all cursor-pointer ${i === 1 ? 'border-blue-500 bg-blue-500/10' : 'border-transparent hover:bg-white/5'}`}>
                <p className={` tracking-tight ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Global Logistics Inc.</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`lg:col-span-8 rounded-[35px] border overflow-y-auto custom-scrollbar p-8 ${theme === 'dark' ? 'bg-slate-900/60 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
          
          <div className="flex items-center gap-3 mb-8">
            <div className="p-3 bg-blue-600/10 rounded-2xl"><UserPlus className="text-blue-600" size={24} /></div>
            <div>
              <h3 className={`text-2xl tracking-tight ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Client Info</h3>
              <p className="text-[14px] tracking-widest text-gray-300">Profile & Contact Configuration</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <h4 className="text-[16px] tracking-[0.3em] text-blue-600 font-bold">Firm Identity</h4>
              <InputField label="Short Name" placeholder="e.g. GLI" theme={theme} />
              <InputField label="Full Name" placeholder="e.g. Global Logistics Inc." theme={theme} />
            ...
            {/* Address Info */}
            <div className="space-y-4">
              <h4 className="text-[16px] tracking-[0.3em] text-blue-600 font-bold">Corporate Address</h4>
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2"><InputField label="Address line 1" theme={theme} /></div>
                <div className="col-span-2"><InputField label="Address line 2" theme={theme} /></div>
          ...
          <div className="space-y-8">
            <div className="flex justify-between items-center">
              <h4 className="text-[16px] tracking-[0.3em] text-green-500">Point of Contact</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <DynamicContactList label="Email Addresses" icon={<Mail size={14}/>} type="email" theme={theme} />
                <DynamicContactList label="Phone Numbers" icon={<Phone size={14}/>} type="tel" theme={theme} />
              </div>
            </div>
