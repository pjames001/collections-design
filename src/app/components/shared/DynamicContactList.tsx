...
export const DynamicContactList: React.FC<{ label: string; icon: React.ReactNode; type: string; theme: string }> = ({ label, icon, type, theme }) => (
  <div className="space-y-0.5">
    <div className="flex justify-between items-center">
      <label className={`text-md tracking-widest ${theme === "dark" ? "text-sky-300" : "text-blue-600"}`}>{label}</label>
    ...
    <div className="space-y-2">
      <div className="relative group">
        ...
        <input 
          type={type} 
          className={`w-full pl-10 pr-10 py-2.5 text-xs font-bold rounded-xl border ${theme === 'dark' ? 'bg-white/5 border-white/10 text-white' : 'bg-slate-50 border-slate-400'}`} 
